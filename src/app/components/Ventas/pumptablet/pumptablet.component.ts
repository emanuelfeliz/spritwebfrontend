import {
  ...
  PopupType,
} from "./../../../services/popupProvider.service";
import {
  Component,
  OnInit,
  ViewChildren,
...
import { PumpPumpTabletComponent } from "./components/pump-pump-tablet/pump-pump-tablet.component";
import { HelperServiceService } from "../../../services/helper-service.service";
import { environment } from "environments/environment";

declare var $;
@Component({
  selector: "app-pumptablet",
  templateUrl: "./pumptablet.component.html",
})
export class PumptabletComponent implements OnInit, AfterViewInit {
  DispensadoresInfo: {
    list: Array<number>;
    showLoading: boolean;
    total: number;
  ...
  @ViewChildren(PumpPumpTabletComponent)
  components: QueryList<PumpPumpTabletComponent>;
  ladosSeleccionados: Array<LadoPT> = [];
  ...
  constructor(
    ...
    private pumptabletService: PumptabletService,
  ...
  ngAfterViewInit(): void {
    $.connection.hub.url = `${environment.Urls.HubUrl}/signalr`;
    const chat = $.connection.myHub;
    if (typeof chat === "undefined") {
      return;
    }
    chat.client.pumpStatus = (pump, state) => {
      this.ResumenesLados.find((x) => x.Lado === Number(pump)).Status = state;
    };
  ...
  imprimirApertura = (aperturaTurno: AperturaTurnoBombero) => {
    this.printer.generateReporte(
      new ReporteRecibido(
        "Reporte de Apertura",
        "~/Reportes/AperturaTurnos/rptAperturaTurno.rpt",
        "apertura_turno",
        false,
        null,
        null,
        JSON.stringify(aperturaTurno)
      ),
      false
    );
  };
  ...
  imprimirCierreLados = (cierreDispensador: AperturaTurnoBombero) => {
    this.printer.generateReporte(
      new ReporteRecibido(
        "Reporte de Cierre de lados",
        "~/Reportes/CierreLados/rptCierreLados.rpt",
        "cierre_lados",
        false,
        null,
        null,
        JSON.stringify(cierreDispensador)
      ),
      false
    );
  };
  ...
  aperturarLadosSeleccionados = () => {
    ...
    if (!canOpen) {
      this.popupProviderService.SimpleMessage(
        ...
        `Ha seleccionado lados que ya estan aperturados`,
        PopupType.WARNING
      );
      return;
    }
    ...
    this.popupProviderService.QuestionMessage(
      "Aperturar lados seleccionados",
      `Esta seguro de aperturar los lados (${ladosPorComa})?`,
      PopupType.QUESTION,
      "Si",
      "No",
      () => {
        this.AutenticadorBomberosService.requestBomberoAutentication(
          ...
          (result) => {
            this.popupProviderService.QuestionMessage(
              `Bombero identificado ${result.bombero.bombero}`,
              ...
              () => {
                ...
                const apertura: AperturaTurnoBombero = new AperturaTurnoBombero(
                  0,
                  "",
                  "",
                  "",
                ...
                apertura.detalles_por_lado = this.GenerarDetallesPorLado(
                  apertura.lados
                );
  ...
  cerrarLadosSeleccionados = () => {
    ...
    this.popupProviderService.QuestionMessage(
      ...
      `Esta seguro de cerrar los lados (${ladosPorComa})?`,
      PopupType.QUESTION,
      "Si",
      "No",
      () => {
        ...
        this.pumptabletService
          .CerrarConjuntoLados(lados)
          .then((result) => {
            this.loading = false;
            this.ladosSeleccionados = [];
            if (result.Success) {
              this.popupProviderService.SimpleMessage(
                "Cerrando lados seleccionados",
                "Lados seleccionados cerrados exitosamente",
                PopupType.SUCCESS
              );
              ...
              this.popupProviderService.SimpleMessage(
                "Cerrando lados seleccionados",
                `No se pudieron los lados seleccionados <br>${result.PossibleError}`,
                PopupType.ERROR
              );
            }
          })
          .catch((error) => {
            ...
            this.popupProviderService.SimpleMessage(
              "Cerrando lados seleccionados",
              "No se pudieron cerrar los lados seleccionados",
              PopupType.ERROR
            );
            this.ladosSeleccionados = [];
          });
  ...
  toggleSeleccionTodos = (): void => {
    ...
    if (this.checkAll) {
      this.ResumenesLados.forEach((ResumenLado) => {
        const ladoPT: LadoPT = new LadoPT(
          true,
          ResumenLado.Lado,
          ResumenLado.TurnoActual,
          ResumenLado.aperturado ? "A" : "C"
        );
        this.ladosSeleccionados.push(ladoPT);
        this._ladosSeleccionados.push(ladoPT);
      });
    } else {
      this.ladosSeleccionados = [];
    ...
    this.components.forEach((child) => {
      setTimeout(() => child.setCheckStatus(this.checkAll), 100);
    });
  };
  selectCheckByPump = (data: LadoPT) => {
    if (data["selected"] == true) {
      ...
      this.ladosSeleccionados.splice(
        this.ladosSeleccionados.findIndex((x) => x.lado == data["lado"]),
        1
      );
    }
  };
  ...
  CerrarDia = () => {
    this.autenticadorFirmantesService.requestFirmanteAutentication(
      this.authData.responseAuth.Response.cerrar_dia,
      "No tiene permisos para cerrar día",
      "Pumptablet",
      () => {
        this.popupProviderService.QuestionMessage(
          "Cerrando Día",
          ...
          "NO",
          () => {
            this.pumptabletService.CerrarDia().then((result) => {
              if (result.Success) {
                this.popupProviderService.SimpleMessage(
                  "Cerrando Día",
                  "El día fue cerrado",
  ...
  CerrarDispensador = () => {
    this.autenticadorFirmantesService.requestFirmanteAutentication(
      this.authData.responseAuth.Response.cerrar_por_dispensador,
      "No tiene permisos de cerrar por dispensador",
      "Pumptablet",
      () => {
        this.popupProviderService.QuestionMessage(
          "Cerrando Dispensador",
          "Está seguro de cerrar el dispensador",
          PopupType.QUESTION,
          "Si",
          "NO",
          () => {
            this.pumptabletService
              .closeDispenser(this.LA, this.TAA, this.LB, this.TAB)
              .then((result) => {
                if (result.Success) {
                  this.changeDispenser();
                  this.printer.openNewTab(
                    `WebForms/CierreDispensador.aspx?turno=${this.TAA}&turno2=${this.TAB}&lado=${this.LA}&lado2=${this.LB}&dispensador=${this.dispenserSelected}`,
                    "Cierre Dispensador"
                  );
                  this.pumptabletService
                    ...
                    .catch((error) => {
                      this.popupProviderService.SimpleMessage(
                        "Desactivando apertura",
                        "Ocurrio un error desactivando apertura",
                        PopupType.ERROR
                      );
                    });
                  this.cargarResumenesLados();
                  ...
                  this.popupProviderService.SimpleMessage(
                    "Cerrando Dispensador",
                    `Dispensador ${this.dispenserSelected} cerrado`,
                    PopupType.SUCCESS
                  );
                } else {
                  this.popupProviderService.SimpleMessage(
                    "Cerrando Dispensador",
                    `No se pudo cerrar el dispensador <br>${result.Response}`,
                    PopupType.ERROR
                  );
                }
              });
  ...
  OnCerrarLadoSuccess = (
    ...
    return new Promise((resolve, reject) => {
      if (print) {
        this.printer.openNewTab(
          `WebForms/TicketPumpTablet.aspx?turno=${turnoACerrar}&lado=${ladoACerrar}`,
          "Cierre de Lado"
        );
      }
  ...
  abrirAperturasAutomaticas = (lados: Array<Lado>): void => {
    if (this.authData.responseAuth.Response.func_abrir_cerrar_seleccionados) {
      this.pumptabletService
        .abrirAperturaBomberoMaestro(lados)
        .then((response) => {
          if (response.Success) {
            if (response.PossibleError !== "") {
              ...
              this.popupProviderService.SimpleMessage(
                "Pumptablet",
                "Lados cerrados y Aperturas realizadas automáticamente",
                PopupType.SUCCESS
              );
            }
            this.changeDispenser();
  ...
  CerrarLado = (ladoStructure) => {
    this.autenticadorFirmantesService.requestFirmanteAutentication(
      this.authData.responseAuth.Response.cerrar_por_lado,
      "No tiene permisos de cerrar por lado",
      "Pumptablet",
      () => {
        this.popupProviderService.QuestionMessage(
          "Cerrando Lado",
          `Está seguro de cerrar el lado ${ladoStructure.lado} del dispensador ${this.dispenserSelected}`,
          PopupType.QUESTION,
          "Si",
          "NO",
          () => {
            ...
            this.pumptabletService
              .closePump(ladoACerrar, turnoACerrar, ladoStructure.ignorarC)
              .then((result) => {
                if (result.Success) {
                  this.popupProviderService.SimpleMessage(
                    "Cerrando Lado",
                  ...
                  if (result.Response == "No_ventas") {
                    this.popupProviderService.QuestionMessage(
                      "Cerrando Lado",
                      `El Lado ${ladoACerrar} no tiene ventas, desea cerrarlo de todas formas?`,
                      PopupType.WARNING,
                      "SI!",
                      "NO!",
                      () => {
                        this.pumptabletService
                          .closePump(ladoACerrar, turnoACerrar, true)
                          .then((result) => {
                            if (result.Success) {
                              this.popupProviderService.SimpleMessage(
                                "Cerrando Lado",
                              ...
                              this.OnCerrarLadoSuccess(
                                ...
                                const lados: Array<Lado> = [];
                                lados.push(
                                  new Lado(ladoACerrar, turnoACerrar + 1)
                                );
                                this.abrirAperturasAutomaticas(lados);
                              });
                            } else {
  ...
  changeDispenser = () => {
    ...
    this.pumptabletService
      .getPumpTabletDataByDispenser(this.dispenserSelected)
      .then((result) => {
      ...
      .catch((error) => {
        this.popupProviderService.SimpleMessage(
          "Cambio dispensador",
          "Ocurrio un error obteniendo data de un dispensador",
          PopupType.ERROR
        );
        this.loading = false;
      });
  ...
  getDispensers = () => {
    this.helperServiceService
      .loadModelList(
        this.DispensadoresInfo,
        this.pumptabletService.getDispensers()
      )
      .catch((err) =>
        this.popupProviderService.SimpleMessage(
          "Error",
          `Error obteniendo los dispensadores ${err}`,
          PopupType.ERROR
        )
      );
  };
      ...
      () => {
        this.validateCloseHour(
          ...
          () => {
            this.popupProviderService.SimpleMessage(
              "PumpTablet",
              "La hora de cerrar no ha llegado, Autentiquese con permiso firmante",
              PopupType.ERROR
            );
            if (!this.authData.responseAuth.Response.is_bombero) {
              this.CerrarLado(ladoStructure);
              return;
            }
            this.autenticadorFirmantesService.requestFirmanteAutentication(
              this.authData.responseAuth.Response.permiso_firmante,
              "No tiene permisos de cerrar por lado",
              "Pumptablet",
              () => {
                this.CerrarLado(ladoStructure);
  ...
  authBombero(success: () => void, errorCallback: () => void): void {
    ...
    this.popupProviderService.QuestionMessage(
      "Cerrar lados seleccionados",
      `Estas seguro de cerrar los lados (${ladosPorComa})?`,
      PopupType.QUESTION,
      "Si",
      "No",
      () => {
        this.AutenticadorBomberosService.requestBomberoAutentication(
          ...
          (result) => {
            ...
            this.aperturaTurnoBomberosActivos.forEach((x) => {
              if (
                x.bombero.toLowerCase() === result.bombero.bombero.toLowerCase()
              ) {
            ...
            if (validation === true) {
              ...
              this.ladosSeleccionados.forEach((dt) => {
                bombero.detalles_por_lado.push(
                  new DetallePorLado(dt.lado, dt.turno)
                );
              });
            }
            ...
            if (validation === false) {
              this.popupProviderService.SimpleMessage(
                "PumpTablet",
                "No eres el bombero que aperturo el turno",
                PopupType.ERROR
              );
              return;
            }
  ...
  closeDispenser(): void {
    ...
    this.validateCloseHour(
      ...
      () => {
        ...
        if (!this.authData.responseAuth.Response.autenticar_cierre_turno) {
          this.CerrarDispensador();
          return;
        }
        this.autenticadorFirmantesService.requestFirmanteAutentication(
  ...
  checkIfHaveOtherTurnOpen(
    ...
    if (!this.authData.responseAuth.Response.bombero_open_more_than_one_turn) {
      this.pumptabletService
        .checkIfHaveAnotherTurnOpen(id_bombero, bombero)
        .then((response) => {
          if (response.Success) {
            success();
          } else {
  ...
  UpdatePumpResumenLados() {
    for (let index = 0; index < this.ResumenesLados.length; index++) {
      this.ResumenesLados[index].pump = {
        pumpNo: 0,
        saleProgress: 0,
        status: "IDLE",
        salePrice: 0,
        volume: 0,
        grade: { blue: 0, green: 0, red: 0, rbg: "", description: "" },
      };
    }
  }
}
