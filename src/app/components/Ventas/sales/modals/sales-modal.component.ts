...
export class SaleModalComponent extends
    ...
    seleccionarComprobante2 = (i: number, tp: TipoComprobante): void => {

      for (let index = 0; index <= this.TIPOS_COMPROBANTES.length; index++) {
        $('#btnAbrirTipoPago' + (index)).removeClass('btn-success');
      }
      $('#btnAbrirTipoPago'+i).addClass('btn-success');
      this.comprobanteDataModel.codigo_ncf = tp.codigo;
